#!/bin/bash
################################################################################
# Complete Build and Integration Script for llcuda CUDA 12 Binaries
#
# This script:
# 1. Builds llama.cpp with CUDA 12 for your target GPU
# 2. Integrates binaries into llcuda package structure
...
LLCUDA_DIR="${PROJECT_ROOT}/llcuda"
LLCUDA_PKG_DIR="${LLCUDA_DIR}/llcuda"
...
show_header "llcuda CUDA 12 Build and Integration"
...
echo "  llcuda pkg:  $LLCUDA_PKG_DIR"
...
show_step "Checking llcuda package..."
if [ ! -d "$LLCUDA_PKG_DIR" ]; then
    show_error "llcuda package not found at $LLCUDA_PKG_DIR"
    exit 1
fi
show_success "llcuda package found"
echo ""
...
show_header "Step 2: Create llcuda Directory Structure"
...
show_header "Step 5: Integrate Binaries into llcuda"
...
echo ""
cat << EOF
# llcuda CUDA 12 Configuration for ${GPU_NAME}
...
#!/usr/bin/env python3
"""
llcuda Integration Test Script
...
LLCUDA_DIR = PROJECT_ROOT / "llcuda" / "llcuda"
...
try:
    import llcuda
    print(f"  ✓ llcuda v{llcuda.__version__} imported successfully")
...
print()
print("Next steps:")
print("  1. Install llcuda in development mode:")
print(f"     cd {PROJECT_ROOT / 'llcuda'}")
print("     pip install -e .")
print()
print("  2. Test with a model (requires model download):")
print("     python3 -c \"import llcuda; engine = llcuda.InferenceEngine(); print(engine)\"")
print()
PYEOF
...
echo -e "${GREEN}✓ Binaries integrated into llcuda package${NC}"
...
echo -e "${YELLOW}How llcuda will find llama-server:${NC}"
...
echo "  4. Cache directory: ~/.cache/llcuda/"
