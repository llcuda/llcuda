"""
llcuda.core - Core tensor and device management
...
import llcuda_cpp
from llcuda_cpp import get_device_properties, get_device_count


# Re-export C++ types
DType = llcuda_cpp.DType
Device = llcuda_cpp.Device
DeviceProperties = llcuda_cpp.DeviceProperties
...
class Tensor:
    ...
    Example:
        >>> import llcuda
        >>> x = llcuda.Tensor([2, 3], dtype=llcuda.DType.Float32, device=0)
        >>> print(x.shape)
        [2, 3]
    """
    ...
    def __init__(self, shape: List[int], dtype: DType = DType.Float32, device: int = 0):
        ...
        """
        self._tensor = llcuda_cpp.Tensor(shape, dtype, device)
    ...
    def from_numpy(cls, arr: np.ndarray, device: int = 0) -> 'Tensor':
        ...
        Returns:
            New Tensor object
        """
        # Map NumPy dtype to llcuda DType
        dtype_map = {
    ...
    def zeros(cls, shape: List[int], dtype: DType = DType.Float32, device: int = 0) -> 'Tensor':
        ...
        t = cls.__new__(cls)
        t._tensor = llcuda_cpp.Tensor.zeros(shape, dtype, device)
        return t
    ...
    def ones(cls, shape: List[int], dtype: DType = DType.Float32, device: int = 0) -> 'Tensor':
        ...
        t = cls.__new__(cls)
        t._tensor = llcuda_cpp.Tensor.ones(shape, dtype, device)
        return t
    ...
    def __matmul__(self, other: 'Tensor') -> 'Tensor':
        ...
        result._tensor = llcuda_cpp.ops.matmul(self._tensor, other._tensor)
        return result
