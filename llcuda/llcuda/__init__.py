...
if _LLAMA_SERVER.exists():
    ...
    # ========================================================================
    # HYBRID BOOTSTRAP: Download binaries ONLY on first import
    # ========================================================================
    try:
        from ._internal.bootstrap import bootstrap_binaries_only  # Use new function
        bootstrap_binaries_only()  # Only downloads binaries
        
        # Re-apply env vars after bootstrap
        if _LIB_DIR.exists():
            _lib_path_str = str(_LIB_DIR.absolute())
            _current_ld_path = os.environ.get('LD_LIBRARY_PATH', '')
            if _lib_path_str not in _current_ld_path:
...
__version__ = "1.1.2"  # Multi-GPU architecture support, Colab/Kaggle compatibility
__all__ = [
    # Core classes
    'InferenceEngine',
    'InferResult',
