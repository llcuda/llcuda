...
# Configuration
GITHUB_RELEASE_URL = "https://github.com/waqasm86/llcuda/releases/download/v1.1.0-runtime"
HF_REPO_ID = "waqasm86/llcuda-models"
BINARY_BUNDLE_NAME = "llcuda-bins-multiarch.tar.gz"
...
def bootstrap_binaries_only() -> None:
    """
    Bootstrap only binaries - no model download.
    ...
    if llama_server.exists() and llama_server.stat().st_size > 0:
        # Already have binaries
        return

    print("=" * 60)
    print("🎯 llcuda First-Time Setup (Binaries Only)")
    print("=" * 60)
    print()
    ...
    try:
        ...
        print("  engine.load_model('gemma-3-1b-Q4_K_M')  # Downloads model when needed")
...
def download_model_if_needed(model_name: str) -> bool:
    """
    Download a specific model if it doesn't exist locally.
    
    Args:
        model_name: Model name (e.g., 'gemma-3-1b-Q4_K_M')
        
    Returns:
        True if model exists or was downloaded, False otherwise
    """
    if not HF_AVAILABLE:
        print(f"⚠️  huggingface_hub not available, cannot download model: {model_name}")
        print("   Install with: pip install huggingface_hub")
        return False
    
    # Map model names to HF filenames (you might want to expand this)
    model_map = {
        'gemma-3-1b-Q4_K_M': 'google_gemma-3-1b-it-Q4_K_M.gguf',
        # Add more model mappings as needed
    }
    
    hf_filename = model_map.get(model_name)
    if not hf_filename:
        print(f"⚠️  Model '{model_name}' not in registry")
        return False
    
    model_file = MODELS_DIR / hf_filename
    if model_file.exists() and model_file.stat().st_size > 100_000_000:  # > 100 MB
        return True
...
def bootstrap(include_model: bool = False) -> None:
    """
    Main bootstrap function with optional model download.
    
    Args:
        include_model: If True, download default model (default: False)
    """
    # Check if binaries already exist
    llama_server = BINARIES_DIR / "cuda12" / "llama-server"
    ...
    if not llama_server.exists():
        ...
        bootstrap_binaries_only()
    
    # Download default model only if requested
    if include_model:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="llcuda bootstrap utility")
    parser.add_argument("--with-model", action="store_true", 
                       help="Download default model along with binaries")
    parser.add_argument("--model-only", action="store_true",
                       help="Download model only (requires existing binaries)")
    ...
    if args.model_only:
        ...
        bootstrap(include_model=True)
    else:
        bootstrap_binaries_only()  # Default: binaries only
[file content end]