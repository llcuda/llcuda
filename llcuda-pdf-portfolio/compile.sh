#!/bin/bash
# Compilation script for llcuda v2.2.0 portfolio PDF

set -e  # Exit on error

echo "========================================"
echo "llcuda v2.2.0 Portfolio PDF Compilation"
echo "========================================"
echo ""
...
pdflatex -interaction=nonstopmode llcuda_v2_2_0_portfolio.tex > /dev/null 2>&1
...
# Show PDF info
if [ -f "llcuda_v2_2_0_portfolio.pdf" ]; then
    echo "Output file: llcuda_v2_2_0_portfolio.pdf"
    ls -lh llcuda_v2_2_0_portfolio.pdf
    echo ""

    # Show page count if pdfinfo is available
    if command -v pdfinfo &> /dev/null; then
        pages=$(pdfinfo llcuda_v2_2_0_portfolio.pdf 2>/dev/null | grep "Pages:" | awk '{print $2}')
        if [ -n "$pages" ]; then
