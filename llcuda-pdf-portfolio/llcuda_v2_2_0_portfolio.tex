...
\hypersetup{
    ...
    pdftitle={llcuda v2.2.0 Portfolio - Waqas Muhammad},
...
\fancyhead[L]{\textcolor{primaryblue}{\small llcuda v2.2.0 Portfolio}}
\fancyhead[R]{\textcolor{darkblue}{\small Waqas Muhammad}}
\fancyfoot[C]{\textcolor{gray}{\small Page \thepage}}
\fancyfoot[R]{\textcolor{primaryblue}{\small llcuda.github.io}}
...
\begin{titlepage}
    ...
    {\Huge\bfseries\textcolor{primaryblue}{llcuda v2.2.0}\par}
    \vspace{0.3cm}
    ...
    \begin{tcolorbox}[colback=lightgray,colframe=gray,width=0.75\textwidth,arc=2mm]
        ...
        \href{https://llcuda.github.io}{llcuda.github.io}
    \end{tcolorbox}
...
\begin{itemize}[nosep]
    ...
    \item Open-source with MIT license, actively maintained at \href{https://github.com/llcuda/llcuda}{github.com/llcuda/llcuda}
\end{itemize}
\end{tcolorbox}
...
llcuda v2.2.0 introduces a novel \textbf{split-GPU architecture pattern} optimized for Kaggle's dual T4 environment. This design enables simultaneous LLM inference and GPU-accelerated visualization without resource contention.
...
\begin{tikzpicture}[
    ...
    external/.style={box, fill=primaryblue!15, draw=primaryblue!60, line width=1pt},
    arrow/.style={->, >=stealth, thick, line width=1.2pt}
]

% Title
\node[above, font=\Large\bfseries] at (0,7.5) {Kaggle Dual Tesla T4 Architecture};

% GPU 0 Column (Left)
\node[above, font=\large\bfseries, gpu0color] at (-3.5,6.5) {GPU 0 (15GB VRAM)};
\node[gpu0] (llama) at (-3.5,5.5) {llama-server\\Port 8090};
\node[gpu0] (model) at (-3.5,4) {Llama-3.2-3B\\Q4\_K\_M};
\node[gpu0] (inference) at (-3.5,2.5) {LLM Inference\\tensor\_split: 1.0};

% GPU 1 Column (Right)
\node[above, font=\large\bfseries, gpu1color] at (3.5,6.5) {GPU 1 (15GB VRAM)};
\node[gpu1] (rapids) at (3.5,5.5) {RAPIDS\\cuDF \& cuGraph};
\node[gpu1] (graphistry) at (3.5,4) {Graphistry\\Visualization};
\node[gpu1] (analytics) at (3.5,2.5) {PageRank\\Analytics};

% Vertical arrows within columns
\draw[arrow, gpu0color!80] (llama) -- (model);
\draw[arrow, gpu0color!80] (model) -- (inference);
\draw[arrow, gpu1color!80] (rapids) -- (graphistry);
\draw[arrow, gpu1color!80] (graphistry) -- (analytics);

% Cross-GPU data flow
\draw[arrow, primaryblue] (inference.east) -- ++(0.8,0) |- node[above, pos=0.25, font=\tiny] {Extract} (rapids.west);

% External connections
\node[external] (hf) at (-3.5,0.8) {HuggingFace Hub};
\node[external] (cloud) at (3.5,0.8) {Graphistry Cloud};

\draw[arrow, primaryblue!60] (hf) -- (model);
\draw[arrow, primaryblue!60] (analytics) -- (cloud);

% Bottom labels
\node[below, text width=3cm, align=center, font=\footnotesize] at (-3.5,0) {\textbf{1.88 GB model}\\28 layers, 896 heads};
\node[below, text width=3cm, align=center, font=\footnotesize] at (3.5,0) {\textbf{929 nodes}\\981 edges};

\end{tikzpicture}
\caption{llcuda v2.2.0 Split-GPU Architecture on Kaggle Dual T4}
\end{figure}

\subsection{Design Principles}

\begin{enumerate}
    \item \textbf{GPU Isolation via CUDA\_VISIBLE\_DEVICES}: Processes explicitly assigned to specific GPUs to prevent memory conflicts
    \item \textbf{Memory Efficiency}: LLM uses 2-5GB on GPU 0, leaving GPU 1 entirely free for visualization workloads
    \item \textbf{API-Driven Communication}: llama-server exposes OpenAI-compatible REST API for architecture queries
    \item \textbf{Production-Ready Error Handling}: Automatic GPU detection, health monitoring, graceful degradation
\end{enumerate}
...
\hline
\textbf{Compute} & CUDA 12.x, Tesla T4 (SM 7.5), FlashAttention, Tensor Cores \\
\hline
\textbf{LLM Backend} & llama.cpp (build 7760), llama-server, GGUF format, NVIDIA NCCL \\
\hline
\textbf{GPU Analytics} & RAPIDS cuDF 25.6, cuGraph (PageRank, Betweenness Centrality), CuPy \\
\hline
\textbf{Visualization} & Graphistry 0.50.4, PyGraphistry API, cloud rendering \\
\hline
\textbf{ML Ecosystem} & Unsloth, HuggingFace Hub, transformers, PyTorch \\
\hline
\textbf{SDK} & Python 3.11+, OpenAI-compatible client, type hints, asyncio \\
\hline
\textbf{Platform} & Kaggle notebooks (12-hour sessions), GitHub Actions CI/CD \\
\hline
\end{tabularx}
\caption{llcuda v2.2.0 Comprehensive Technology Stack}
\end{table}
...
\begin{tikzpicture}[
    ...
    level/.style={rectangle, draw, thick, rounded corners, minimum width=2.5cm, minimum height=0.8cm, align=center, fill=white},
    beginner/.style={level, fill=gpu0color!15},
    intermediate/.style={level, fill=gpu1color!15},
    advanced/.style={level, fill=primaryblue!15}
]

% Title
\node[above] at (0,5.5) {\Large\textbf{11-Notebook Tutorial Progression}};

% Beginner Level
\node[above, font=\large\bfseries] at (-3.5,4.5) {Beginner};
\node[beginner] (nb01) at (-3.5,4) {\small 01: Quick Start};
\node[beginner] (nb02) at (-3.5,3) {\small 02: Server Setup};
\node[beginner] (nb03) at (-3.5,2) {\small 03: Multi-GPU};

% Intermediate Level
\node[above, font=\large\bfseries] at (0,4.5) {Intermediate};
\node[intermediate] (nb04) at (0,4) {\small 04: GGUF Quant};
\node[intermediate] (nb05) at (0,3) {\small 05: Unsloth};
\node[intermediate] (nb06) at (0,2) {\small 06: Split-GPU};
\node[intermediate] (nb07) at (0,1) {\small 07: Knowledge Graph};

% Advanced Level
\node[above, font=\large\bfseries] at (3.5,4.5) {Advanced};
\node[advanced] (nb08) at (3.5,4) {\small 08: Doc Network};
\node[advanced] (nb09) at (3.5,3) {\small 09: Large Models};
\node[advanced] (nb10) at (3.5,2) {\small 10: Complete Flow};

% Flagship
\node[advanced, fill=accentblue!30, minimum width=3.5cm, minimum height=1.2cm, line width=1.5pt] (nb11) at (0,-0.5) {\textbf{11: GGUF Visualization}\\FLAGSHIP};

% Arrows
\draw[->, thick, gpu0color] (nb01) -- (nb02);
\draw[->, thick, gpu0color] (nb02) -- (nb03);
\draw[->, thick, gpu1color] (nb03) -- (nb04);
\draw[->, thick, gpu1color] (nb04) -- (nb05);
\draw[->, thick, gpu1color] (nb05) -- (nb06);
\draw[->, thick, gpu1color] (nb06) -- (nb07);
\draw[->, thick, primaryblue] (nb07) -- (nb08);
\draw[->, thick, primaryblue] (nb08) -- (nb09);
\draw[->, thick, primaryblue] (nb09) -- (nb10);
\draw[->, thick, accentblue, line width=2pt] (nb10) -- (nb11);
...
\begin{tikzpicture}[
    node distance=1.5cm,
    box/.style={rectangle, draw, thick, rounded corners, minimum width=4cm, minimum height=1cm, align=center, font=\small},
...
% Title
\node[above, font=\Large\bfseries] at (0,8) {Notebook 11: Six-Phase Workflow};

% Phase 1: Setup (centered at top)
\node[setup] (phase1) at (0,6.5) {\textbf{Phase 1}\\Setup \& GPU Detection};

% Left column: GPU 0 workflow
\node[above, font=\large\bfseries, gpu0color] at (-3.5,5.3) {GPU 0 Processing};
\node[gpu0] (phase2) at (-3.5,4) {\textbf{Phase 2}\\Model Serving\\llama-server};

% Right column: Extraction and Analytics
\node[above, font=\large\bfseries, primaryblue] at (3.5,5.3) {Data Pipeline};
\node[process] (phase3) at (3.5,4) {\textbf{Phase 3}\\Architecture\\Extraction (929 nodes)};
\node[gpu1] (phase4) at (3.5,2.3) {\textbf{Phase 4}\\GPU Analytics (GPU 1)\\PageRank \& Centrality};
\node[process] (phase5) at (3.5,0.6) {\textbf{Phase 5}\\Generate 8 Graphistry\\Dashboards};

% Phase 6: Output (centered at bottom)
\node[output] (phase6) at (0,-1) {\textbf{Phase 6}\\HTML Dashboard + Statistics};

% Vertical arrows within columns
\draw[arrow, primaryblue] (phase3) -- (phase4);
\draw[arrow, primaryblue] (phase4) -- (phase5);

% Arrows from Phase 1
\draw[arrow, gray!60] (phase1) -- (phase2);
\draw[arrow, gray!60] (phase1) -- (phase3);

% Cross-column data flow
\draw[arrow, gpu0color!80] (phase2.east) -- ++(0.5,0) |- (phase3.west);

% Arrows to Phase 6
\draw[arrow, primaryblue] (phase5) -- (phase6);
...
\node[box, fill=gpu0color!20, line width=1pt, draw=gpu0color!80] (llcuda) at (0,8) {\textbf{llcuda (GPU 0)}\\LLM Inference};
...
\vspace{0.2cm}
\textbf{Primary:} \texttt{pip install git+https://github.com/llcuda/llcuda.git@v2.2.0}\\
\textbf{Mirror:} HuggingFace at \texttt{waqasm86/llcuda}\\
...
\hline
\end{tabularx}
\caption{llcuda v2.2.0 API Design Principles}
\end{table}

\subsection{Documentation \& Learning Resources}

\textbf{Comprehensive Documentation Site:} \href{https://llcuda.github.io}{llcuda.github.io}
...
\begin{tcolorbox}[colback=lightgray,colframe=darkblue]
\textbf{Repository:} \href{https://github.com/llcuda/llcuda}{github.com/llcuda/llcuda}\\
...
\hline
\end{tabularx}
\caption{llcuda v2.2.0: Cross-Domain Impact}
\end{table}
...
\Large\textbf{llcuda v2.2.0 - Official Links}

\vspace{0.3cm}
\normalsize
\textbf{Documentation:} \href{https://llcuda.github.io}{https://llcuda.github.io}\\[0.2cm]
\textbf{GitHub Repository:} \href{https://github.com/llcuda/llcuda}{https://github.com/llcuda/llcuda}\\[0.2cm]
\textbf{Tutorial Notebooks:} \href{https://llcuda.github.io/tutorials/}{https://llcuda.github.io/tutorials/}\\[0.2cm]
\textbf{Quick Start Guide:} \href{https://llcuda.github.io/guides/quickstart/}{https://llcuda.github.io/guides/quickstart/}\\[0.2cm]
\textbf{API Reference:} \href{https://llcuda.github.io/api/overview/}{https://llcuda.github.io/api/overview/}
\end{tcolorbox}
...
All 11 notebooks available at: \href{https://llcuda.github.io/tutorials/}{llcuda.github.io/tutorials/}
...
\textbf{Website:} & \href{https://llcuda.github.io}{llcuda.github.io} \\
...
\subsection{Why llcuda v2.2.0 Matters}
...
\begin{tcolorbox}[colback=white,colframe=accentblue,width=0.9\textwidth,arc=2mm]
\centering
\textit{llcuda v2.2.0 represents the intersection of GPU systems engineering, ML infrastructure, and AI explainability—demonstrating both technical depth and practical impact on free, accessible compute infrastructure.}
