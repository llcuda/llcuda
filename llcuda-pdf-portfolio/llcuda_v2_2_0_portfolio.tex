...
% Title
\node[above, font=\Large\bfseries] at (0,8.5) {Kaggle Dual Tesla T4 Architecture};

% GPU 0 Column (Left)
\node[above, font=\large\bfseries, gpu0color] at (-4,7) {GPU 0 (15GB VRAM)};
\node[gpu0] (llama) at (-4,6) {llama-server\\Port 8090};
\node[gpu0] (model) at (-4,4.3) {Llama-3.2-3B\\Q4\_K\_M};
\node[gpu0] (inference) at (-4,2.6) {LLM Inference\\tensor\_split: 1.0};

% GPU 1 Column (Right)
\node[above, font=\large\bfseries, gpu1color] at (4,7) {GPU 1 (15GB VRAM)};
\node[gpu1] (rapids) at (4,6) {RAPIDS\\cuDF \& cuGraph};
\node[gpu1] (graphistry) at (4,4.3) {Graphistry\\Visualization};
\node[gpu1] (analytics) at (4,2.6) {PageRank\\Analytics};
...
% External connections - positioned lower to avoid overlaps
\node[external] (hf) at (-4,0.5) {HuggingFace Hub};
\node[external] (cloud) at (4,0.5) {Graphistry Cloud};

% HuggingFace Hub arrow - routed to LEFT side to avoid overlaps
\draw[arrow, primaryblue!60] (hf.north) -- (hf.north |- model.south) -- (model.south);

% Cross-GPU data flow - routed ABOVE to avoid overlapping nodes
\draw[arrow, primaryblue] (inference.east) -- ++(1.5,0) node[above, pos=0.5, font=\tiny] {Extract} |- (rapids.west);

% Graphistry Cloud arrow - straight down
\draw[arrow, primaryblue!60] (analytics.south) -- (analytics.south |- cloud.north) -- (cloud.north);

% Bottom labels - positioned below external nodes
\node[below, text width=3.5cm, align=center, font=\footnotesize] at (-4,-0.5) {\textbf{1.88 GB model}\\28 layers, 896 heads};
\node[below, text width=3.5cm, align=center, font=\footnotesize] at (4,-0.5) {\textbf{929 nodes}\\981 edges};
